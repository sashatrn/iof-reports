...
function printUsage(logger: Logger): void {
  logger.info(
    "Usage: node dist/index.js watch --input-dir <dir> --output-dir <dir> --report <side-by-side-individual|team|side-by-side-relay|rogaining|rogaining-awards|rogaining-diplomas|rogaining-score|rogaining-splits|military-individual|military-relay> [--config config.json] [--courses courses.xml] [--poll-ms 3000] [--settle-ms 1000] [--port 4173] [--diploma-template off|on]",
  );
}
  ...
  for (let i = 3; i < argv.length; i += 1) {
    ...
    if (arg === "--report") {
      if (!value || !isWatchReportType(value)) {
        logger.error(
          { report: value },
          "Invalid watch report type. Expected one of: side-by-side-individual, team, side-by-side-relay, rogaining, rogaining-awards, rogaining-diplomas, rogaining-score, rogaining-splits, military-individual, military-relay.",
        );
        printUsage(logger);
        process.exit(1);
      }
